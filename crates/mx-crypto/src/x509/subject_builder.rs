...
use crate::x509::OID_EMAIL_ADDRESS;
...
impl SubjectBuilder {
    pub fn country(mut self, country_name: &str) -> Self {
        self.0
            .append_printable_string(Oid(OID_COUNTRY_NAME.as_ref().into()), country_name)
            .unwrap();
        self
    }
    ...
    pub fn locality(mut self, locality: &str) -> Self {
        self.0
            .append_utf8_string(Oid(OID_LOCALITY_NAME.as_ref().into()), locality)
            .unwrap();
        self
    }
    ...
    pub fn organization(mut self, organization: &str) -> Self {
        self.0
            .append_utf8_string(Oid(OID_ORGANIZATION_NAME.as_ref().into()), organization)
            .unwrap();
        self
    }
    ...
    pub fn common_name(mut self, common_name: &str) -> Self {
        self.0
            .append_utf8_string(Oid(OID_COMMON_NAME.as_ref().into()), common_name)
            .unwrap();
        self
    }
    ...
    pub fn email_address(mut self, email: &str) -> Self {
        self.0
            .append_utf8_string(Oid(OID_EMAIL_ADDRESS.as_ref().into()), email)
            .unwrap();
        self
    }
