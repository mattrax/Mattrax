...
pub fn mount<T: Application>() -> Router<Arc<T>> {
    Router::new()
        ...
        .route("/Discovery.svc", post(|State(app): State<Arc<T>>, body: String| async move {
            ...
            let enrollment_domain = app.enrollment_domain();
                ...
                body: DiscoverResponseBody {
                    discover_result: DiscoverResponseDiscoverResponse {
                        ...
                        discover_result: DiscoverResponseDiscoverResult {
                            ...
                            authentication_service_url: match app.determine_authentication_method() {
                                crate::Authentication::Federated { url } => Some(url.to_string()),
                            },
                        }
                    }
                },
	...
	// }
        ...
        .route("/Enrollment.svc", post(|State(app): State<Arc<T>>, body: String| async move {
            ...
            let auth = match cmd.header.security {
    ...
    match app.verify_authentication(token.to_string()) {
                                Ok(Some(result)) => result,
                                Ok(None) => {
                                     // fault.Fault(err, "the users authenticity could not be verified", soap.FaultCodeAuthentication)
                                     error!("todo: proper soap fault. invalid auth");
                                     return StatusCode::UNAUTHORIZED.into_response();
                                }
                                Err(err) => {
                                    // TODO: Internal server error fault
                                    error!("todo: proper soap fault. error verifying auth: '{err:?}'");
                                    return StatusCode::INTERNAL_SERVER_ERROR.into_response();
                                },
                            }
                        },
            ...
            let upn = Some("todo@example.com".to_string()); // TODO: Work this out properly
            ...
            // TODO: What should we do if you try and enroll over another device???
            ...
            let (identity_cert, identity_key) = app.identity_keypair();
            let certificate = csr.signed_by(&identity_cert, &identity_key).unwrap();  // TODO: Error handling
            ...
            hasher.update(identity_cert.der());
            ...
            let root_certificate_der = BASE64_STANDARD.encode(identity_cert.der());
            ...
            // TODO: <parm name="EntDMID" value="{mdm_device_id}" datatype="string" /> -> Goes above `SyncApplicationVersion`
