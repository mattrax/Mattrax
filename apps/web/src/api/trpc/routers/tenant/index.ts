	...
	delete: tenantProcedure.mutation(async ({ ctx }) => {
		...
		await db.transaction(async (db) => {
			...
			const policy_assignable = db
				...
				.where(eq(policies.tenantPk, ctx.tenant.pk));
			const policy_assignable_query = db.$with("inner").as(policy_assignable);
			await db.execute(
				sql`with ${policy_assignable_query} as ${policy_assignable} delete from ${policyAssignables} using ${policyAssignables} join ${policy_assignable_query} on ${policyAssignables.policyPk} = ${policy_assignable_query.id};`,
			);
