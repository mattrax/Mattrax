...
export const msRouter = new Hono<HonoEnv>()
	.get(
		...
		async (c) => {
			...
			const params = new URLSearchParams({
				...
				prompt: "login",
				redirect_uri: `${env.VITE_PROD_ORIGIN}/api/ms/link`,
	...
	.get(
		...
		async (c) => {
			...
			try {
				const url = new URL(env.VITE_PROD_ORIGIN);
			...
			if (!skipSubscription) {
				await msGraphClient(entraTenantId)
					...
					.post({
						...
						notificationUrl: `${env.VITE_PROD_ORIGIN}/api/webhook/microsoft-graph`,
						lifecycleNotificationUrl: `${env.VITE_PROD_ORIGIN}/api/webhook/microsoft-graph/lifecycle`,
						resource: "/users",
