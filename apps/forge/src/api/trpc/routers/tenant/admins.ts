...
export const adminsRouter = createTRPCRouter({
	...
	sendInvite: tenantProcedure
		...
		.mutation(async ({ ctx, input }) => {
			...
			// try {
			await getDb().insert(tenantAccountInvites).values({
	...
	acceptInvite: publicProcedure
		...
		.mutation(async ({ ctx, input }) => {
			const invite = await getDb().query.tenantAccountInvites.findFirst({
	...
	invites: tenantProcedure.query(async ({ ctx }) => {
		return await getDb().query.tenantAccountInvites.findMany({
