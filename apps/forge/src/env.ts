...
import { H3Event, getEvent } from "vinxi/http";
import { z } from "zod";
...
function create(runtimeConfig: any) {
	return createEnv({
		server: {
...
export const getEnv = () => {
	try {
		const event = getEvent() as H3Event;
		return create(event.context.cloudflare.env);
	} catch {
		return create(process.env);
	}
};
