...
new aws.ec2.RouteTableAssociation("MattraxMDMVPCSubnetRouteTableAssociation", {
...
const tailscaleKey = new tailscale.TailnetKey("MattraxMDMServerTailscaleKey", {
	preauthorized: true,
	reusable: true,
	ephemeral: true,
	tags: ["tag:prod"],
...
const userData = $resolve([tailscaleKey.key, secrets.DatabaseURL.value]).apply(
...
new aws.ec2.Eip("MattraxMDMServerEIP", {
	domain: "vpc",
...
new cloudflare.Record("MattraxMDMServerDNSIPv6", {
	...
	zoneId: domainZone.id,
	name: "mdm",
	value: instance.ipv6Addresses[0],
});
